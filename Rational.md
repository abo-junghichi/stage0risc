...
リトルエンディアンのメモリモデルとなる。
現在流通している実マシンはバイト単位のメモリアクセスに優れているので、
[RAVM: compile once and run anywhere
](https://zsmith.co/ravm.php)
や
[Virtual machine showdown: Stack versus registers
](https://dl.acm.org/doi/10.1145/1328195.1328197)
に倣い、フィールド幅はバイト単位。
また、個々のレジスタ毎に別々の回路を用意する実マシンと異なり、
レジスタの性質が均質でなければ仮想マシンの性能が落ちるので、
ゼロレジスタは排除。
...
MIPS Release 6やANS Forthに倣い、
比較演算命令の返り値は(0,1)ではなく(0,~0(=-1))にする。
